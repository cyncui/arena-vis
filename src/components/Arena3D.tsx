...
function getImageUrl(block: ArenaBlock): string | null {
  return block.image?.small?.src || block.image?.src || null;
}
...
export default function Arena3D({ initialSlug }: Arena3DProps) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const dir = camera.getWorldDirection(new (require('three').Vector3)());
        ...
        if (res.ok && data.graphData) {
          setGraphData(prev => {
            ...
            return {
              ...
              links: [...prev.links, ...data.graphData.links],
            };
          });
        }
      } else {
  ...
  const nodeThreeObject = useCallback((node: any) => {
    ...
    const mat = new THREE.MeshStandardMaterial({
      color: graphNode.id === selectedNode?.id ? 0xffd93d : 0x95e1d3,
      emissive: graphNode.id === selectedNode?.id ? 0xffd93d : 0x95e1d3,
  ...
  return (
    <div className="relative w-full h-screen">
      ...
      {selectedNode && (
        <div className="absolute top-4 right-4 z-10 w-80 bg-black/50 border border-white/20 rounded-sm backdrop-blur-sm flex flex-col transition-all">
          ...
          {!sidebarMinimized && (
            <>
              ...
              <div className="px-4 py-3 border-t border-white/10">
                <button
                  onClick={() => { setSelectedNode(null); setHistory([]); setSidebarMinimized(false); }}
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 text-white/40 font-pixel text-center">
        <p>Drag to rotate · Right-drag to pan · Scroll to zoom · WASD to fly · Q/E up/down</p>
      </div>
    </div>
  );
}
